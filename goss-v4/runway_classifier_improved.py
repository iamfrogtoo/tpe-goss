...
def extract_features(trajectory):
    ...
    # 提取降落阶段数据（高度从高到低）
    landing_phase = sorted(trajectory, key=lambda x: x['altitude'], reverse=True)
    landing_phase = [point for point in landing_phase if point['altitude'] > 0 and point['altitude'] < 10000]
    ...
    speed_changes = [speeds[i] - speeds[i+1] for i in range(len(speeds)-1)]
    ...
    # 5. 轨迹长度和时间
    trajectory_length = 0
    for i in range(len(landing_phase)-1):
        lat1, lon1 = map(float, landing_phase[i]['position'].split(','))
        lat2, lon2 = map(float, landing_phase[i+1]['position'].split(','))
        trajectory_length += calculate_distance(lat1, lon1, lat2, lon2)
    
    # 6. 降落时间
    timestamps = [point['timestamp'] for point in landing_phase]
